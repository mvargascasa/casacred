...
class ApiController extends Controller
{
    ...
    public function listActivatedProperties(Request $request)
    {
        ...
        if ($request->has('property_type') && $request->property_type != null) {
            $query->where('listingtype', $request->input('property_type'));
        }
        ...
        if ($request->has('listing_title') && $request->listing_title != null) {
            ...
            $query->where(function ($q) use ($propertyTypeIds, $statusTypes, $locationKeywords) {
                ...
                // Si se encontraron IDs de tipos de propiedad, aplicar filtro
                if (!empty($propertyTypeIds)) {
                    $q->whereIn('listingtype', $propertyTypeIds);
                }

                // Aplicar filtro de estado (venta o renta) fuera del where anidado
                if (!empty($statusTypes)) {
                    $q->whereIn('listingtypestatus', $statusTypes);
                }

                // Aplicar filtros de ubicación si hay palabras clave
    ...
    {
        $propertie = Listing::where('slug', $slug)
            ->where('status', 1)
            ->where('available', 1)
            ->first();
